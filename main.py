...
# with postgres("snap_analytics") as con:  # creates connection or gives you connection
#     df = pd.read_sql(
#         f'''select *
#         from snapanalytics.auto_test_20230615''', con)
# title = str(df.loc[0, "title"])
# first_name = str(df.loc[0, "first_name"])
# last_name = str(df.loc[0, "last_name"])
# birth_dt = str(df.loc[0, "birth_dt"])
# email = str(df.loc[0, "email"])
# mobile = str(df.loc[0, "mobile"])
# post_code = str(df.loc[0, "post_code"])



# C:\Users\pbadcock\OneDrive - Snap Finance, LLC\Documents\GitHub\snapstar


# opts = webdriver.ChromeOptions()
# opts.add_argument('--window-size = 5020px, 5020px') # defining width and height
url = "https://admin-portal.lumapp.io/login"
driver = webdriver.Chrome() #options = opts)
# Sets window size
driver.set_window_size(1050, 850)
# Goes to pre defined url
driver.get(url)
time.sleep(1)

# entering username
driver.find_element(By.XPATH,"/html/body/div/div[2]/div/div/form/div[2]/div[1]/input").send_keys('pbadcock')
# entering Password
driver.find_element(By.XPATH,"/html/body/div/div[2]/div/div/form/div[2]/div[2]/input").send_keys('8dfg9B5750!')
